...
void Assembler::initOpcodes() {
    ...
    opcodes["blt"] = BLT; //edited
    opcodes["ada"] = ADA; //edited
}
...
bool Assembler::isSingleOperand(Opcode opcode) {
    if(opcode == B || opcode == BGT || opcode == BEQ || opcode == CALL || opcode == BLT)
        return true;
    return false;
}
...
uint32 Assembler::generateEncoding(Opcode opcode, int reg1, int src1, int src2, bool imm) {
    if(opcode == ADA)
        return imm  ? ((opcode << 27) | (reg1 << 22)  | (reg1 << 18) | (src2 & 0xFFFF) | (1 << 26)) : ((opcode << 27) |  (reg1 << 22) | (reg1 << 18) | (src2 << 14));
    else
