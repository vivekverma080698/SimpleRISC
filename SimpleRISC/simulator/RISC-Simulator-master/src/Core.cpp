...
void Core::write_context() {
	if (specifycontextout){
		...
		if (lt){
			context_file<<"Flags.lt : True"<<endl;
		}
		else {
			context_file<<"Flags.lt : False"<<endl;
		}
...
void Core::decode() {
	...
	if( !(opcode5 == 1 || ( opcode5 == 0 && opcode3 == 1 && opcode1 == 1 && ( opcode4 == 1 || opcode2 == 0) ) ) 	|| 	(opcode5 == 1 && opcode4 == 0 && opcode3 == 0 && opcode2 == 1 && opcode1 == 1)  || (opcode5 == 1 && opcode4 == 0 && opcode3 == 1  && opcode2 == 1 && opcode1 == 1) ){
		temp_isWb = true;
		//pprint(2)<<"isWb ";
	}
	else{
	...
	of_ex.isAda.Write(temp_isAda); //edited
...
void Core::execute() {
	...
	bool temp_isBlt = of_ex.isBlt.Read();
	...
	bool temp_isAda = of_ex.isAda.Read(); //edited
	...
	if(temp_isMul || temp_isDiv || temp_isMod)
	{
		isMDM = true;
	}

	if(isCheck == false && isMDM == true){
		return;
	}
	...
	if (temp_isAda){ //edited
		//pprint(2)<<"ADA operation"<<endl;
		temp_aluResult = temp_A + temp_B;
...
void Core::mem_access() {
	...
	bool temp_isBlt = ex_ma.isBlt.Read();
	...
	bool temp_isAda = ex_ma.isAda.Read();
	...
	ma_rw.isAda.Write(temp_isAda);  //edited
...
void Core::write_back() {
	...
	bool temp_isBlt = ma_rw.isBlt.Read();
	...
	bool temp_isAda = ma_rw.isAda.Read();  //edited
...
bool Core::checkValidPC(unsigned int testPC){
	...
	if (inst_bitset(testInst,28, 32) < 24) {
		return true;
	}
...
bool Core::check_data_conflict(PipelineRegister& A, PipelineRegister& B){
	...
	if (B_opcode5 == 1 && B_opcode4 == 0 && B_opcode3 == 1 && B_opcode2 == 1 && B_opcode1 == 0){
		return false;	//B is blt  edited
	}
...
string Core::disassemble (unsigned int inst_word){
	...
	if (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 1){
		inst = "ADA" + modifier + " " + registerstring(rd) + ", " + registerstring(rs1);
		if (isImmediate){
			inst += ", " + hexstring(imm);
		}
		else {
	...
	if (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0){
		inst = "BLT <" + sintstring(offset,27) + ">"; //edited
	}
